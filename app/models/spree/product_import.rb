  ...
  class ProductImport < ActiveRecord::Base
    ...
    def create_product_using(params_hash)
      ...
      unless product.valid?
        ...
        log("{product.errors.full_messages}")
        return false
      end
    ...
    def associate_product_with_taxon(product, taxonomy, taxon_hierarchy)
      ...
      puts "taxonomy.root is #{taxonomy.root}"

      taxon_hierarchy.split(/\s*\&\s*/).each do |hierarchy|
        hierarchy = hierarchy.split(/\s*>\s*/)
        last_taxon = taxonomy.root
        hierarchy.each do |taxon|
          log "~~~~find or create #{taxon}, #{taxonomy.id}"
          child = last_taxon.children.find_by_name_and_taxonomy_id(taxon, taxonomy.id)
          if(child == nil) 
            puts 'creating....'
            child = last_taxon.children.create(name: taxon, taxonomy_id: taxonomy.id)
            puts "created??? #{child.valid?}"
          end
          last_taxon = child

          puts "now last_taxon is #{last_taxon.inspect}"
          puts "children.count is #{last_taxon.children.count}"
          log "~~~~done"
        end
